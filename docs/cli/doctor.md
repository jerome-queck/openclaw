...
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local TUI clients still running. Doctor reports their process ids but does not terminate a TUI whose installation cannot be established; close the listed sessions, then rerun doctor.
